...
OverlayBackground {
    ...
    function applyLimit() {
        ...
        var cfgPaths = ["~/.config/MangoHud/MangoHud.conf"]; // MangoHud config files
        var updateCommands = cfgPaths.map((path) => {
            return "if grep -q '^fps_limit=' " + path + "; " + "then sed -i 's/^fps_limit=.*/fps_limit=" + fpsValue + "/' " + path + "; " + "else echo 'fps_limit=" + fpsValue + "' >> " + path + "; fi";
        }).join("; ");
        ...
        IconToolbarButton {
            ...
            text: {
                ...
                case FpsLimiterContent.State.Error:
                    return "close";
                case FpsLimiterContent.State.Success:
                    return "check";
